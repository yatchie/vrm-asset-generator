  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 160, right: 20, background: 'rgba(0,0,0,0.8)', padding: '15px 20px', borderRadius: 8, width: 350, zIndex: 10, border: '1px solid #555' }}>
        <h3 style={{marginTop: 0, fontSize: 16, borderBottom: '1px solid #444', paddingBottom: 8}}>Setting for: {adjustTarget} <span style={{fontSize: 10, color: '#777', fontWeight: 'normal'}}>(v1.4.4)</span></h3>
        ...
        <div style={{marginTop: 15, borderTop: '1px solid #444', paddingTop: 10}}>
           ...
           <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', fontSize: 12, margin: '5px 0'}}>
             ...
             <input type="range" min="0" max="2.0" step="0.01" value={globalOutlineWidth} onChange={e => setGlobalOutlineWidth(parseFloat(e.target.value))} style={{flex: 1, margin: '0 10px'}} />
             <span style={{width: 40, textAlign: 'right'}}>{globalOutlineWidth.toFixed(2)}</span>
           </div>
        </div>
      </div>
